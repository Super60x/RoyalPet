...
- [ ] Client-side upload validatie (jpg/png/webp, max 10MB, min resolutie)
- [ ] Replicate FLUX.1 Pro API aangeroepen met config-based Renaissance prompt
- [ ] Rate limiting op `/api/generate` (max 5/IP/uur)
- [ ] Client-side polling elke 2-3 sec via `/api/generate/status`
- [ ] Progress bar met roterende wachtteksten (~60 sec)
- [ ] Portret opgeslagen in `portraits` tabel met 6-karakter slug
- [ ] Watermarked versie naar `portraits-public`, clean naar `portraits-private`
- [ ] Redirect naar `/preview/[id]` werkend
- [ ] Error handling bij slechte foto of API fout
- [ ] Learnings opgeslagen → `learnings/dag-02.md`

### Sessie 3 — Preview + Prijzen + Kaders (~2 uur)
- [ ] Sharp watermark op server gegenereerd
- [ ] Watermarked preview zichtbaar op `/preview/[id]`
